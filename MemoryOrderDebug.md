...
> x64의 강한 메모리 모델은 하드웨어 레벨 재배치는 줄여준다.  
> 하지만 여기서 발생했던 문제는 Debug 빌드임에도 발생한 문제이다.  
> 컴파일러 최적화에 의한 재배치는 없었지만, 실행 중 __CPU 코어에서 비순차 실행__ 에 의해 명령 재배치가 발생한 것이다.  
> 즉, 플랫폼(x64)와 빌드 모드(Debug)에 관계없이 멀티코어 환경에서는 명시적인 메모리 배리어 설정이 필수적인 것을 확인했다.    
