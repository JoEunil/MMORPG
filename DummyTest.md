...
### 5.4  분석 결과
본 장애는 서버의 기능적 결함이 아닌, 테스트 환경의 시스템 리소스 고갈에 따른 I/O 처리 불능 상태로 인해 발생하였다.  
- 서버측 데이터 송신 정상 확인
...
__결론__  
테스트 환경의 물리적 리소스(4 Core) 한계에 의한 시스템 붕괴 현상이다.
CPU 부하가 I/O 처리 속도를 저하시켰고, 이것이 `클라이언트의 수신 버퍼 고갈(Zero Window) → 
