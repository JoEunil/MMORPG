...
### 해결해야 할 Pain Point
- 동기화 비용: Writer와 Reader가 동일 자원에 접근 시 Mutex를 사용하면 심각한 성능 저하 발생.
- 데이터 일관성: 전송 도중 데이터가 변경되면 안 되며, 항상 '특정 시점에 완성된' 스냅샷이 필요.
- 메모리 효율성 (vs Object Pool):
...
- 잘못된 데이터: Zone 객체는 여러 개인데 ThreadPool은 공유됩니다. Worker가 가진 로컬 버퍼가 현재 처리하려는 Zone의 것이 아니거나, 이미 문맥이 바뀐 Zone의 데이터를 들고 있어 논리적 오류를 유발합니다.
