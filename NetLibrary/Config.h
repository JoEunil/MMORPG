...
namespace Net {
	...
	inline constexpr uint32_t TARGET_PACKETPOOL_SIZE = 5000;

	inline constexpr uint32_t MAX_BPACKETPOOL_SIZE = 10000;
	inline constexpr uint32_t MIN_BPACKETPOOL_SIZE = 200;
	inline constexpr uint32_t TARGET_BPACKETPOOL_SIZE = 5000;
	...
	inline constexpr uint16_t RECV_BUFFER_SIZE = 128;  
	inline constexpr uint32_t RING_BUFFER_SIZE_MIN = RECV_BUFFER_SIZE* 256; // recv buffer size * 16으로 설정했을 때 dummy client 테스트에서 buffer override 발생..
	inline constexpr uint32_t RING_BUFFER_SIZE = NextPowerOf2(RING_BUFFER_SIZE_MIN);
	inline constexpr uint32_t RING_BUFFER_SIZE_MASK = RING_BUFFER_SIZE - 1;
	inline constexpr uint16_t MAX_PACKET_LEN = 4096; // PacketPool에서 미리 할당할 패킷 크기, 대용량 상태 동기화 패킷 처리를 위해 4KB로 설정

	inline constexpr uint16_t RELEASE_Q_SIZE_MIN = RING_BUFFER_SIZE / sizeof(Core::PacketHeader) + 2;

	inline constexpr uint16_t RELEASE_Q_SIZE = NextPowerOf2(RELEASE_Q_SIZE_MIN);
	inline constexpr uint16_t RELEASE_Q_SIZE_MASK = RELEASE_Q_SIZE - 1;

	inline constexpr uint16_t SESSION_SHARD_SIZE = 16;
	inline constexpr uint16_t SESSION_SHARD_MASK = SESSION_SHARD_SIZE-1;
	inline constexpr uint8_t PING_COUNT_LIMIT = 5;

	inline constexpr std::chrono::seconds PING_LOOP_WAIT = std::chrono::seconds(1);


	constexpr uint32_t RECV_WINDOW = 20;
	constexpr uint32_t BYTE_THRESHOLD = RECV_WINDOW* RECV_BUFFER_SIZE * 0.8;
	constexpr uint32_t MIN_BYTE_PER_WINDOW = sizeof(Core::PacketHeader) * RECV_WINDOW * 0.8;
