...
## 논블로킹 소켓
논블로킹 소켓이란, I/O 함수 호출 시 블로킹하지 않고 즉시 반환되는 소켓을 의미합니다.  
...
```
- 소켓을 논블로킹 모드로 설정하면 recv/send 호출 자체는 동기적입니다.
- 호출 시점에 데이터가 준비되지 않으면 바로 반환 → -1 + EAGAIN/EWOULDBLOCK
- 즉, 커널이 pending 상태를 관리하지 않고, 애플리케이션에서 재시도나 이벤트 기반 처리 필요

## 리눅스의 비동기 IO
리눅스는 모든 시스템 자원을 파일 디스크립터(FD)로 관리합니다.  
소켓도 FD로 관리되며, 비동기 I/O 처리는 __논블로킹 소켓__을 기반으로 합니다.  
...
## IOCP와 epoll의 차이
epoll: 논블로킹 소켓 특성상 Send 호출 시 송신 버퍼가 가득 찬 경우 EAGAIN/EWOULDBLOCK이 발생합니다.
→ 애플리케이션이 재시도 로직을 구현해야 함.

IOCP: OVERLAPPED 구조체를 통한 비동기 I/O 관리가 필요합니다.
→ Send 호출 자체가 비동기적이므로 재시도 로직이 불필요하며, 커널이 내부 송신 큐를 관리하여 거의 실패하지 않습니다.
