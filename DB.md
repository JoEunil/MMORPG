...
그러나 커넥션 풀을 DB별로 분리하여, 트랜잭션이 DB 경계를 넘지 못하도록 의도적으로 설계했다.  
...
두 DB가 하나의 트랜잭션으로 묶이지 않으므로, Outbox·Inbox 패턴으로 배송이 exactly-once에 수렴하도록 한다.
전달 자체는 at-least-once이고, Inbox의 dedup이 중복 적용을 걸러 최종 효과가 정확히 한 번이 된다.  
...
Outbox가 남아 있는 한 배송은 계속 재시도되고, 이미 처리한 event는 Inbox에서 걸러진다. 크래시 지점과 무관하게 최종적으로 정확히 한 번 지급된 상태로 수렴한다.
...
> 배송의 Write-ahead 기록은 별도 로그 파일이 아니라 Outbox 행 자체이다.  
> DB의 자체 redo log가 각 INSERT/UPDATE의 durability를 보장한다.  
>
> 이 문서의 "Write-ahead"는 MySQL 엔진 자체의 redo log를 가리킨다. Game DB 인벤토리 캐시가 자체적으로 갖는 앱 레벨 WAL(별도 로그 파일 + LSN + Replay)은 이와 별개 메커니즘이며 [WAL.md](WAL.md)에서 다룬다.
...
- 쿼리 추상화 — 애플리케이션 코드 단순화
- 유지보수 — 조건 변경 시 View 한 곳만 수정하면 됨
- 민감 데이터 노출 제한 — 필요한 컬럼만 노출하여 불필요한 데이터 접근 차단
