...
graceful shutdown은 서버 종료 시 **진행 중인 작업을 유실 없이 마무리하고, 자원을 안전한 순서로 정리**하는 것을 말한다.  
...
이후 ASan([ASAN.md](ASAN.md))으로 더미 클라이언트 2000명 부하 테스트를 돌리면서 종료 시 발생하던 크래시/데이터 유실을 하나씩 잡아 **자원 정리 순서를 다시 정립**했다.
...
이 순서의 근거는 파이프라인이 **producer-consumer 체인**이라는 데 있다 — net(입력 생산) → core(입력 소비·DB write 생산) → cache(DB write 소비).   
각 P-C 경계에서 **producer(상류)를 먼저 종료**해야, consumer(하류)가 "더 이상 새 입력이 없는" 상태에서 이미 받은 것만 drain하고 안전하게 끝낼 수 있다.  
순서가 뒤집히면 멈춘 큐로 producer가 계속 밀어넣어 **유실**되거나, 새 입력이 끊이지 않아 **drain이 끝나지 않는다**.  

> **P-C 역전 두 곳** — 둘 다 유실 없이 처리했다.
...
3단계에서 큐에 넣고 4단계에서 실제로 기록되며, 각 단계의 큐/워커가 **drain**하므로 유실이 없다.   
...
ASan 부하 테스트를 돌리며 발견한 종료 관련 문제들. 검출 수단은 **ASan(메모리 오류)** 과 **종료 로그 관찰**이 섞여 있어 각 항목에 표시했다.
...
- 조치: 각 Stop을 **idempotent + 로거 null-safe**로 변경  
...
- 조치: **큐가 비고 running=false일 때만 종료**하도록 변경 → 남은 작업 전부 처리 후 exit
...
1. **Stop/CleanUp은 idempotent** — `running.exchange(false)`로 재호출을 무시. 명시적 호출 경로와 소멸자가 이중 실행돼도 안전해야 한다.
2. **종료 시 로거는 null-safe** — 소멸자 경로에서 전역 로거가 이미 reset됐을 수 있으므로 `if (logger)` 가드.
3. **소멸 순서 = 자원 제공자가 소비자보다 나중에** — 자원을 보유하는 쪽(소비자)을 먼저 선언(먼저 소멸), 자원을 제공하는 풀을 나중에 선언(나중에 소멸). 멤버 선언 순서로 강제되므로 순서 변경 시 주석 필수.
4. **로거 파괴는 spdlog 스레드풀보다 먼저** — async 로거가 살아있는 풀로 flush하도록.  
5. **큐/워커의 종료 정책은 명시** — hard stop(남은 것 버림) vs graceful drain(비울 때까지 처리). DB write처럼 유실이 곧 데이터 손실인 경로는 **반드시 drain**.
6. **P-C 경계에선 producer를 먼저 종료** — 종료 순서는 producer-consumer 관계로 결정된다. 파이프라인(net → core → cache, 그리고 내부의 recvMQ → dbWorker)의 각 경계에서 상류(producer)를 먼저 멈춰 새 입력을 끊은 뒤, 하류(consumer)가 남은 것을 drain하고 종료한다. 순서가 뒤집히면 유실 또는 무한 대기 → **규칙 5의 drain이 성립하기 위한 전제**다.
