...
거래 유실 방지는 WAL의 몫이 아니다. 그건 [Bazaar.md](Bazaar.md)의 **Outbox/Inbox**가 캐시-DB 경계를 넘는 배송을 exactly-once로 맡는다.  
WAL은 그 경계 안쪽, 즉 **캐시 자체의 유실 창**을 담당한다. 드랍·사용·장착처럼 거래 밖에서 일어나는 인벤토리 변경은 Outbox/Inbox가 다루지 않는다.  
