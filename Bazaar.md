...
- 구매 트랜잭션(sp_bazaar_buy)이 characters_diamond과 bazaar 테이블을 단일 트랜잭션으로 처리하므로, 두 테이블이 동일 DB 인스턴스에 존재해야 한다. 
	- 이로 인해 재화 테이블과 거래소 테이블의 독립적인 샤딩이 불가능하다.
	- 통합 서버 수가 증가할수록 단일 DB 인스턴스에 트랜잭션이 집중되어 처리량 병목이 발생할 수 있다.
